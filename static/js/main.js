document.addEventListener('DOMContentLoaded', function() {
    ...
    function appendChatMessage(sender, text) {
        ...
        msgDiv.className = sender === 'user' ? 'flex justify-end mb-2.5' : 'flex justify-start mb-2.5';

        if (sender === 'user') {
            msgDiv.innerHTML = `<div class="bg-blue-600 text-white rounded-2xl px-5 py-2.5 max-w-[80%] text-sm shadow-sm font-medium leading-relaxed">${escapeHtml(text)}</div>`;
        } else if (sender === 'loading') {
            msgDiv.id = 'chat-loading-bubble';
            msgDiv.innerHTML = `<div class="bg-gray-100 text-gray-600 rounded-2xl px-5 py-2.5 text-xs animate-pulse font-medium border border-gray-200">Thinking with Patient Assistant AI...</div>`;
        } else {
            let htmlContent = '';
            try {
            ...
            msgDiv.innerHTML = `<div class="bg-gray-100 text-gray-800 rounded-2xl px-5 py-3 max-w-[85%] text-xs shadow-sm border border-gray-200 leading-relaxed prose">${htmlContent}</div>`;
        }
    ...
    async function sendPatientChatMessage() {
        ...
        // Determine current patient name if available
        let patientId = 'irani_gangadharappa';
        const currentAnalysis = tabResults[activeTab];
        if (currentAnalysis && currentAnalysis.patient_name) {
            patientId = currentAnalysis.patient_name;
        }
        ...
        try {
            ...
            if (!response.ok) {
                appendChatMessage('bot', 'Network error while connecting to Patient Assistant AI.');
                return;
            }
            ...
            if (data.success && data.response) {
                ...
                appendChatMessage('bot', data.error || 'Network error while connecting to Patient Assistant AI.');
            }
        } catch (err) {
    ...
    if (sendChatBtn && chatInput) {
        sendChatBtn.addEventListener('click', sendPatientChatMessage);
